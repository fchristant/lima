...
import HueLightControlPicker from '@components/Light/Modify/LightControl/HueLightControlPicker';

// add custom CSS property type checking
 type MyCustomCSS = CSSProperties & Record<`--${string}`, number | string>;

 interface HueLightProps {
   light: Bulb
 }

const HueLight = memo(function HueLight({ light }: HueLightProps) {
   ...
   */
   ...
   const { on, reachable, xy, ct, bri, colormode } = state;
  ...
  return (
   ...
   <div className='hue-light' style={cardStyle}>

      {light?.name} ({light?.num})
    
      <div className="hue-light__spot" style={spotStyle}>
         <div className={isRerender.current? ' hue-light--highlight' : ''} key={Math.random()}></div>
      </div>

      <HueLightToggle light={num} on={on} reachable={reachable} />
      <HueLightColorPicker light={num} currentColor={lampColor} enable={xy && isAvailable} />
      <HueLightControlPicker light={num} currentValue={ct} enable={!xy && isAvailable} min={153} max={500} attribute="ct" className="hue-light-brightness-picker" />
      <HueLightControlPicker light={num} currentValue={bri} enable={isAvailable} min={1} max={254} attribute="bri" className="hue-light-temperature-picker" />
      <HueLightControlPicker light={num} currentValue={state.sat} enable={xy && isAvailable} min={0} max={254} attribute="sat" className="hue-light-saturation-picker" />
      <HueLightControlPicker light={num} currentValue={state.hue} enable={xy && isAvailable} min={0} max={65535} attribute="hue" className="hue-light-hue-picker" />
...
function calculateLampColor(state: any) {
   ...
   return '#333333'; // Default color in case none of the above conditions meet.
}
